...
### Inventory images (imagestore)
- Product photos for **inventory catalogue entries** — a **gallery (many images per item)**, offered on the Add form, Edit form, and the click-to-open device "spec card". UI: `frontend/src/components/InventoryImageGallery.vue` (used by `BomGlobalForm.vue` and the `BomGlobalView.vue` detail modal, gated on `inventoryMode` — a BOM-mode `detailCard.id` is a `bom_and_costing` id, not an inventory id).
- Backend `app/api/inventory_images.py`: `POST /api/inventory/:iid/images` (multipart, key `files`, all-or-nothing), `GET /api/inventory/:iid/images`, `GET /api/inventory-images/:id` (served **inline** for `<img>`), `DELETE /api/inventory-images/:id`. Validates `.png/.jpg/.jpeg/.webp/.gif` by extension **and** magic bytes. Files at `IMAGESTORE_DIR/<inventory_id>/<uuid>.<ext>` (default `backend/imagestore`, gitignored); shares `MAX_CONTENT_LENGTH`.
- **Authz is member-level for BOTH upload and delete** (`inventory.create`) — deliberately *not* owner-or-admin like documents, because the catalogue has no owner column. So any user who can add a catalogue entry can also manage its images.
- Blobs are JWT-authed, so the frontend must fetch via `reqBlob` → `URL.createObjectURL` (revoked on unmount); a bare `<img src="/api/...">` gets 401. Deleting an inventory item cascades `inventory_images` rows (FK) and sweeps its `imagestore/<iid>/` folder (`remove_inventory_imagestore`, called after commit in `delete_inventory`). New table → `create_all()` auto-creates it (DDL mirrored in `init_db.sql`).
- **Inventory Excel import** (`parseInventoryCatalogueExcel` in `utils/recordExport.js`) mirrors the BOM import but has **no Project column** and **resolves Category/Type text labels → lookup FK ids** against the lookups store (round-trips with the inventory export); unknown Category/Type cells become row errors. `BomGlobalView` enables import in inventory mode and POSTs one `invStore.createRow` per valid row.
