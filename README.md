...
A full-stack project-tracking application for automation and engineering projects. Built with a **Vue 3 SPA** frontend and a **Flask REST API** backend, with JWT authentication, role-based authorization, CRUD operations, tasks, per-project auxiliary records, rich analytics dashboards, and dark mode.
...
pytest                      # 40 tests — auth, projects, tasks/comments, stats, BOM lists, records
```
...
- **BOM Global view** (`/bom`) — a cross-project BOM inventory (`GET /api/bom/all`) with Excel import (maps rows to projects by name or id), Excel/PDF export, and **saved BOM lists** — named, reusable subsets of BOM rows scoped to a target project (`/api/bom-lists`, CRUD, FK-referenced not snapshotted).
